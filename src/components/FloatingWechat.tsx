...
export default function FloatingWechat() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <button
        onClick={handleClick}
        className="bg-gold text-primary px-5 py-3 rounded-full font-semibold shadow-lg hover:bg-gold-light hover:scale-105 transition-all flex items-center gap-2"
      >
        ...
        <span className="text-sm">{copied ? "已复制!" : "微信: " + profile.wechat}</span>
      </button>
    </div>
  );
}